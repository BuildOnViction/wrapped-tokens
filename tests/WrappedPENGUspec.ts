...
import { WrappedPENGU } from '../typechain-types';
import { ECDSASignature, EIP712Domain, EIP712TypeDefinition } from './EIP712';

const ZERO_ADDRESS = '0x0000000000000000000000000000000000000000';

describe('WrappedMEW token', async function() {
  let owner: Signer;
  ...
  before(async function() {
    ...
    const tokenFactory = await hhe.ethers.getContractFactory('WrappedPENGU');
...
async function createPermit(token: WrappedPENGU, owner: Signer, spenderAddress: string, amount: BigNumber, deadline: BigNumber): Promise<ECDSASignature> {
