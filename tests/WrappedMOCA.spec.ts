...
import { WrappedMOCA } from '../typechain-types';
import { ECDSASignature, EIP712Domain, EIP712TypeDefinition } from './EIP712';

const ZERO_ADDRESS = '0x0000000000000000000000000000000000000000';

describe('WrappedMOCA token', async function() {
  let owner: Signer;
  ...
  before(async function() {
    ...
    const tokenFactory = await hhe.ethers.getContractFactory('WrappedMOCA');
...
async function createPermit(token: WrappedMOCA, owner: Signer, spenderAddress: string, amount: BigNumber, deadline: BigNumber): Promise<ECDSASignature> {
